...
As properties on Busato Road could be accessed from either side of the road, removal of PSCIS `r as.character(my_site)` could be explored as an option for providing access to the 125m of habitat located upstream and below PSCIS `r as.character(my_site)`.  However, to facilitate an estimate of "worst case scenario", costs for replacement of both crossing `r as.character(my_site)` and `r as.character(my_site2)` with bridges are estimated at \$`r format(my_cost_estimate() %>% pull(cost_est_1000s) * 1000, big.mark = ',')` and \$`r format(my_cost_estimate(site = my_site2) %>% pull(cost_est_1000s) * 1000, big.mark = ',')` respectively.  The combined cost benefit of replacements are estimated at `r paste0('$', as.character(my_cost_estimate() %>% pull(cost_net) * 1000 + my_cost_estimate(site = my_site2) %>% pull(cost_net) * 1000))` linear m/\$1000 and and \$`r as.character(my_cost_estimate() %>% pull(cost_area_net) * 1000  + my_cost_estimate(site = my_site2) %>% pull(cost_area_net) * 1000)`m^2^/\$1000.
